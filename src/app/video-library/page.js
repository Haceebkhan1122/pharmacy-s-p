...
import { videoListing } from "@/utils/endpoints";
import { Suspense } from "react";
import Loader from "@/components/loader/Loader";

async function getVideoListingWidgets() {
  const apiLocale = 1;
  ...
  try {
    ...
    const res = await fetch(
      `${process.env.NEXT_PUBLIC_BASE_URL_PAGES}/${videoListing}`,
      {
        method: "GET",
        headers: {
          platform: "web",
...
export default async function LocalePage({ params }) {
  ...
  const videoWidgets = await getVideoListingWidgets();
  const widgets = Array.isArray(videoWidgets?.widgets) ? videoWidgets.widgets : [];
  return (
    <div>
