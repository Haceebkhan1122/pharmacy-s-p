...
import { researchPapers } from "@/utils/endpoints";
import { Suspense } from "react";
import Loader from "@/components/loader/Loader";

async function getResearchWidgets() {
  const apiLocale = 1;
  ...
  try {
    ...
    const res = await fetch(
      `${process.env.NEXT_PUBLIC_BASE_URL_PAGES}/${researchPapers}`,
      {
        method: "GET",
        headers: {
          platform: "web",
...
export default async function LocalePage({ params }) {
  ...
  const researchWidgets = await getResearchWidgets();

  const widgets = Array.isArray(researchWidgets?.widgets) ? researchWidgets.widgets : [];
  return (
    <div>
